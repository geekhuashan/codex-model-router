...
const f=d.refresh;document.getElementById('catalog').textContent=f?'模型目录检查：'+f.checked_at+' · '+f.outcome+' · 本次新增 '+(f.added||[]).length+' · 待配置 '+(f.pending||[]).length+(f.needs_app_restart?' · 目录曾更新；如尚未重启 App/CLI，请重启以加载新菜单':''):'';
